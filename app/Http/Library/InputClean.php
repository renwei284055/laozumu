...
class  InputClean
{
          ...
          switch (is_array($ret)) {
            case true:
              ...
              foreach ($ret as $key => $value) {
                # code...
                $res[$key] =  self::setFilter(self::cleanYellow(self::cleanJsCss( self::cleanJs($value))));
              }
              break;
            ...
            case false:
              ...
              # code...
                $res =  self::setFilter(self::cleanYellow(self::cleanJsCss( self::cleanJs($ret))));
              break;
          }
     ...
     static protected function cleanJs($html)
     {
       $html=trim($html);
      ...
      * Return string
      */
     static protected function cleanJsCss($html){
       $html=trim($html);
     ...
     static private function cleanFilter($html){
       $html=trim($html);
     ...
     //过滤非法的敏感字符串
     static private function cleanYellow($txt){
       $txt=str_replace(
     ...
     //全半角字符替换
     static protected function setFilter($html){
